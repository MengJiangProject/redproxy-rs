...
impl Indexable for (String, u32) {
    ...
    async fn type_of_member(&self, _ctx: ScriptContextRef) -> Result<Type> {
        Ok(Type::Integer)
    }
    fn get(&self, index: i64) -> Result<Value> {
        let n_char_val = self
            ...
            .chars()
            .nth(index as usize)
            .ok_or_else(|| anyhow::anyhow!("index out of range"))? as i64;
        Ok(n_char_val.into())
    }
}
...
impl Accessible for TestNativeFailingAccess {
    ...
    async fn type_of(&self, _name: &str, _ctx: ScriptContextRef) -> Result<Type> {
...
impl Callable for TestNativeFailingCall {
    async fn signature(&self, _ctx: ScriptContextRef, _args: &[Value]) -> Result<Type> {
        Ok(Type::Integer) // Signature is Integer, but call will fail
    }
    async fn call(&self, _ctx: ScriptContextRef, _args: &[Value]) -> Result<Value> {
